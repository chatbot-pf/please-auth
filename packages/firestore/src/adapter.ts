...
import type { CollectionReference, Firestore, Query, Transaction } from 'firebase-admin/firestore'
import type { CollectionNames, FirestoreAdapterConfig } from './types.js'
import type { FieldMapper } from './utils.js'
...
async function findManyWithOr(
  ...
  for (const clause of where) {
    if (clause.connector === 'OR' && groups.at(-1)!.length > 0) {
      groups.push([])
    }
  ...
  return {
    ...
    async delete({ model, where }) {
      ...
      for (const doc of snap.docs) {
        if (clientFilters.length > 0) {
          const record = docToRecord(doc.id, doc.data(), mapper)
          if (!matchesAllClientFilters(record, clientFilters))
            continue
        }
